...
|company.name| form.type| date.filed| quarter |filing.year|
|-----------|-----------|------------|---------|----------|
...
Then, I will trim too rare and too common tokens using the tf-idf values. Finally, I will investigate top 10 words (important keywords) for the three levels. 
...
Q_token <- foreach(i=1:nrow(companies_q), .options.snow = opts, .combine = rbind) %dopar% {
  
  con <- dbConnect(SQLite(), paste0("edgar.sqlite"))  
  this_cik <- companies_q$CIK[i]
  
  this_q <- dbGetQuery(con, paste0("SELECT * FROM `10_Q_token` WHERE cik == ", this_cik))
...
ggplot(top_10_q_industry, aes(x = reorder_within(word, n, GICS), y = n, group = factor(GICS), fill = factor(GICS))) + 
  geom_bar(stat = "identity") + 
  scale_x_reordered() +
...
ggplot(top_10_q_market_sample, aes(x = reorder_within(word, n, GICS_sub), y = n, group = factor(GICS_sub), fill = factor(GICS_sub))) + 
  geom_bar(stat = "identity") + 
  scale_x_reordered() +
...
K_token <- foreach(i=1:nrow(companies_k), .options.snow = opts, .combine = rbind) %dopar% {
  
  con <- dbConnect(SQLite(), paste0("edgar.sqlite"))  
  this_cik <- companies_k$CIK[i]
  
  this_k <- dbGetQuery(con, paste0("SELECT * FROM `10_K_token` WHERE cik == ", this_cik))
...
ggplot(top_10_k, aes(x = reorder(word, n), y = n)) + 
  geom_bar(stat = "identity") + 
  coord_flip() +
  ylab("frequency") +
  xlab("top 10 word")
...
ggplot(top_10_k_industry, aes(x = reorder_within(word, n, GICS), y = n, group = factor(GICS), fill = factor(GICS))) + 
  geom_bar(stat = "identity") + 
  scale_x_reordered() +
  facet_wrap(~ GICS, scales = "free") +
  ylab("frequency") +
...
ggplot(top_10_k_market_sample, aes(x = reorder_within(word, n, GICS_sub), y = n, group = factor(GICS_sub), fill = factor(GICS_sub))) + 
  geom_bar(stat = "identity") + 
  scale_x_reordered() +
  facet_wrap(~ GICS_sub, scales = "free") +
  ylab("frequency") +
...
to_regress_q <- foreach(i = 1:length(cik_list_q), .options.snow = opts, .combine = rbind) %dopar% {
...
to_regress_k <- foreach(i = 1:length(cik_list_k), .options.snow = opts, .combine = rbind) %dopar% {
...
getMgmtdisc_text <- function (this_html_filing) {
    ...
    tryCatch({
      if (is.na(md.dicusssion) == TRUE) {
        startline <- grep("^\\s*[item]*\\s*Management.*Discussion\\s*and\\s*Analysis\\s*of\\s*Financial\\s*Condition.*Operations\\s*$", f.text, ignore.case = TRUE) 
        endline <- grep("^\\s*[item]*\\s*A{1}*\\s*Quantitative\\s*and\\s*Qualitative\\s*Disclosures\\s*$", f.text, ignore.case = TRUE)
        
        # try different pattern
        if (length(startline) == 0) {
          startline <- grep("^\\s*[item]*\\s*Management.*Discussion\\s*and\\s*Analysis\\s*of\\s*Financial\\s*Condition\\s*$", f.text, ignore.case = TRUE) 
        }
        
        # try to find item 8 with title
        # if there is no item 7A
        if (length(endline) == 0) {
          endline <- grep("^\\s*[item]*\\s*Financial\\s*Statements\\s*and\\s*Supplementary\\s*Data\\s*$", f.text, ignore.case = TRUE)
        }
...
for (k in 1:length(cik_list_k)) {
  ...
  mgmt_disc <- foreach(i = 1:length(filings), .options.snow = opts, .combine = rbind) %dopar% {
    ...
    if (is.na(mgmt_disc_text) == FALSE) {
      ...
      # at least for three times
      # e.g. "aaa"
      customstopwords <- tokens_h$word[grepl("\\b(\\S+?)\\1\\1\\S*\\b", tokens_h$word)]
  ...
  print(paste0("Finished parallel processing! It took ", Sys.time()-start_time, " to process"))
...
dbSendQuery(con, "CREATE INDEX cik_idx_mgmt_dics ON 'management_discussion_token' (cik)")
dbSendQuery(con, "CREATE INDEX doc_id_idx_mgmt_dics ON 'management_discussion_token' (doc_id)")
...
postagged_mgmt <- foreach(i = 1:length(all_doc_id), .options.snow = opts, .combine = rbind) %dopar% {
  ...
  fwrite(list(word), file = paste0("C:/r_working_directory/gitgit/edgar/text/word", i,".txt"))
  
  # perform POS tagging
  text.tagged <- treetag(paste0("C:/r_working_directory/gitgit/edgar/text/word", i,".txt"),
                         treetagger = "manual", 
                         lang = "en", 
...
- topic 6:`ground_association_maryland_buyer_definition_formation_utility` (more related to _higher_ level industries in the dictionary)

- topic 5: `pharmacy_medicine_hospital_efficacy_patent_biotechnology_oncology` (more related to _lower_ level industries in the dictionary)

- topic 14: `merchandise_shipment_season_retailer_holiday_opening_specialty` (more related to _higher_ level industries in the dictionary)

These three topics show highest marginal change in probabilities across the industries among all topics. This means that _they seem to appear dominantly in certain industries (exclusitivity of topics with respect to industry sectors)_.
...
- topic 9: `regulator_recommendation_reliability_pollution_carbon_ratemaking_territory` (more related to _lower_ level markets in the dictionary)

- topic 13: `engineer_viability_accretion_hub_quantity_reservoir_revision` (more related to _lower_ level markets in the dictionary)

- topic 6: `ground_association_maryland_buyer_definition_formation_utility` (more related to _higher_ level markets in the dictionary)

These three topics show highest marginal change in probabilities across the markets among all topics. This means that _they seem to appear dominantly in certain markets (exclusitivity of topics with respect to market sectors)_.
...
- Topic 12 (`obsolescence_intellectual_patent_user_cancellation_manufacturer_evidence`) starts to appear very frequently since the start of 2019 as shown in the graph and statistical evidence (`confidence interval and p-value`) also shows that this increase is significant. 

- Topic 27 (`extent_issuance_commitment_job_repayment_enactment_cut`) topic proportion starts to increase since the start of 2017 and statistical evidence (`confidence interval and p-value`) supports this trend. 

- topic 28 (`counsel_view_divestiture_recoverability_circumstance_lawsuit_repatriation`) topic proportion starts to rapidly increase since the start of 2019 and statistical evidence (`confidence interval and p-value`) supports this trend.
...
for_regression <- cbind(metadata,theta)
```

```{r}
# I chose `SentimentLM` as fixed independent variable based on the previous analysis results
...
# figure out which topics have possible predictibility on stock price movement
# firstly try simple linear regression
...
- `disclosure_policy_judgment_interpretation_position_accordance_valuation`: Marginal increase in the proportion (probability) of topic_1 significantly *decrease* the amount of price change after the filing by 0.1997 (t = -2.052, p-value = 0.0403)

- `engineer_viability_accretion_hub_quantity_reservoir_revision`: Marginal increase in the proportion (probability) of topic_13 significantly *decrease* the amount of price change after the filing by 0.29088 (t = -2.557, p-value = 0.0106)

- The overall model fit increased gradually according to `R2` and `Adjusted R2`. However, the model fit is pretty bad as R2 is only 0.004. Therefore, further investigation on more powerful predicting variables are required to increase the model predictibility.
